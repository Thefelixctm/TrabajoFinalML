...
def plot_compare(hip_sz=500, noise=0.1):
    ...
    with open(f'results/IPHNAA_numN{hip_sz}_noise{noise}.data', 'rb') as filehandle:
        iphn = pickle.load(filehandle)

    with open(f'results/MHNAA_numN{hip_sz}_noise{noise}.data', 'rb') as filehandle:
...
def plot_capacity_tests():
    ...
    for t_type in range(3):
        for m in range(3):
            for dt in range(3):
                with open(f'results/AutoA_Model{mds[m]}_ActF{acts[m]}_Test{tests[t_type]}_numN[100, 500, 1000, 2500, 5000, 7500]_noise{noises[t_type]}_frcMsk[0]_data{datas[dt]}.data', 'rb') as filehandle:
...
def plot_corruption_tests(data=2):
    ...
    for t_type in range(5):
        for m_type in range(4):
            with open(f'results/AutoA_Model{m_type}_Test{t_types[t_type]}_numN[1000]_noise{noise[t_type-5]}_frcMsk{frcMsk[t_type-5]}_data{data}.data', 'rb') as filehandle:
                d = pickle.load(filehandle)
...
def plot_sample_tests():
    ...
    for d in range(5):
        for m_type in range(4):
            with open(f'results/AutoA_Model{m_type}_Test3_numN[100, 500, 1000, 2500, 5000, 7500]_noise[0.5]_attnFalse_frcMsk[0.5]_data{d}.data', 'rb') as filehandle:
    ...
    with torch.no_grad():
        ...
        for wu in wus:
            ...
            for hp_sz in hip_szs:
                with open(f'results/AA_Online_Model1_numN{hp_sz}_data0_numData300_upType{wu}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                    axs[it].errorbar(x, dta[0], yerr=dta[1], fmt='o', alpha=.3, label=names[wu], markersize=2)
                    it += 1
        it = 0
        for hp_sz in hip_szs:
            with open(f'resultsa/AA_OnlineBP_numN{hp_sz}_data0_numData300.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
...
def plot_onlineContAbl(max_iter=3000, hid_sz=300, data=6, simf=1):
    with torch.no_grad():
        ...
        for m in range(6):
            with open(f'results/AA_Online_Simf{simf}_numN{hid_sz}_data{data}_numData{max_iter}_upType{dets[m][0]}_det{dets[m][1]}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
            ...
            with open(f'results/AA_OnlineCont_Simf{simf}_numN{hid_sz}_data{data}_numData{max_iter}_upType{dets[m][0]}_det{dets[m][1]}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                x = torch.linspace(1, max_iter, dta[0].size(1))
...
def plot_onlineAndCont(max_iter=3000, hid_sz=300, data=6, simf=1):
    with torch.no_grad():
        ...
        with open(f'results/AA_Online_Simf{simf}_numN{hid_sz}_data{data}_numData{max_iter}_upType{0}_det{3}.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
            x = torch.linspace(1, max_iter, dta[0].size(1))
        ...
        with open(f'results/AA_OnlineCont_Simf{simf}_numN{hid_sz}_data{data}_numData{max_iter}_upType{0}_det{3}.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
            x = torch.linspace(1, max_iter, dta[0].size(1))
        ...
        for o in range(2):
            with open(f'results/AA_OnlineBP_data{data}_opt{o}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                x = torch.linspace(1, max_iter, dta[0].size(1))
            ...
            with open(f'results/AA_OnlineContBP_data{data}_opt{o}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                x = torch.linspace(1, max_iter, dta[0].size(1))
...
#################################################################################################################
def plot_cont(simf=2): # <-- CORRECCIÓN: simf=2 por defecto
    with torch.no_grad():
        ...
        for d in range(2):
            # <-- CORRECCIÓN: 'results/' y simf dinámico
            with open(f'results/AA_OnlineCont_Simf{simf}_numN{hid_sz[d]}_data{data[d]}_numData{max_iter[d]}_upType0_det0.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
            ...
            for o in range(4):
                # <-- CORRECCIÓN: 'results/'
                with open(f'results/AA_OnlineContBP_data{data[d]}_opt{o}_hdsz{hid_sz[d]}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                x = torch.linspace(1, max_iter[d], dta[0].size(1))
        ...
        # ... (resto de la función con las rutas corregidas) ...
        # <-- CORRECCIÓN: 'results/' y simf=2 para ODI
        with open(f'results/AA_OnlineContDom_Simf2_numN1300_numData3000_upType0_det0.data','rb') as filehandle:
            dta = pickle.load(filehandle)
        ...
        for o in range(4):
            # <-- CORRECCIÓN: 'results/'
            with open(f'results/AA_OnlineContDomBP_opt{o}_hdsz1300.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
            x = torch.linspace(1, 3000, dta[0].size(1))
        ...
        filename = os.path.join(output_dir, 'OnCont_L1_Continual.png')
        pylab.savefig(filename)
        print(f"✅ Gráfico guardado localmente en: {filename}")
        pylab.show()
    ...
    with torch.no_grad():
        ...
        for nst in range(2):
            for plus in range(2):
                ...
                std_rec = []
                for nup in range(4):
                    with open(f'results/AA_NoisyOnline_numN{hid_sz}_data{data}_numData{max_iter}_NUp{num_up[nup]}_NsType{nst}_plus{bool(plus)}.data', 'rb') as filehandle:
                        dta = pickle.load(filehandle)
        ...
        for o in range(2):
            for nst in range(2):
                ...
                std_rec = []
                for nup in range(4):
                    with open(f'results/AA_NoisyOnlineBP_data{data}_opt{o}_NUp{num_up[nup]}_NsType{nst}.data','rb') as filehandle:
                        dta = pickle.load(filehandle)
                        y_mse.append(torch.mean(dta[0][:, -1]))
                        std_mse.append(torch.std(dta[0][:, -1]))
                        y_rec.append(torch.mean(dta[1][:, -1]))
...
def plot_tree_aa():
    with torch.no_grad():
        ...
        for d in range(3):
            for t in range(4):
                for n in range(3):
                    with open(f'results/AutoA_Model{m_type[n]}_ActF0_Test{tst[t]}_numN[1000]_noise{x[0]}_frcMsk{x[1]}_data{data[d]}.data', 'rb') as filehandle:
                        dta = pickle.load(filehandle)
...
def plot_online_test(cont=False, data=2, max_iter=5001, wtup=0):
    with torch.no_grad():
        ...
        for h in range(len(hid_szs)):
            with open(f'results/AA_Online_Simf1_numN{hid_szs[h]}_data{data}_numData{max_iter}_upType{wtup}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
        ...
        with open(f'results/AA_Online_TreeArch5_data{data}_numData{max_iter}_wtupType{wtup}.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
            x = torch.linspace(1, max_iter, dta[-2].size(1))
...
def plot_recog():
    with torch.no_grad():
        ...
        for rec_t in range(2):
            with open(f'results/Recogn_Simf1_numN300_data0_numData3000_upType0_recT{rec_t}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
        ...
        g = [70, .5]
        for rec_t in range(2):
            with open(f'results/MHN_Recogn_beta0.05_numN300_data0_numData3000_gamma{g[rec_t]}_rect{rec_t}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
...
def plot_cont_cumul():
    with torch.no_grad():
        fig, axs = pylab.subplots(2, 3, figsize=(7, 4)) # Hice el gráfico un poco más grande para la leyenda
        names = ['MHN-SGD', 'MHN-Adam', 'MHN-EWC++', 'MHN-ER']
        clrs = ['#1f77b4', 'black', 'orange', '#2ca02c']
        lst = [':', ':', ':', ':']
        mrks = ['^', 'o', 's', 'p'] # Marcadores distintos para cada modelo BP
        ...
        # --- Bloque 1: SQHN ---
        for d in range(2): # Bucle para EMNIST y CIFAR-100
            final_acc, final_mse, final_acc_std, final_mse_std = [], [], [], []
            for hdz in range(3):
                with open(f'results/AA_OnlineCont_Simf2_numN{num_hd[d][hdz]}_data{dt[d]}_numData{num_d[d]}_upType0_det0.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
        ...
        final_acc, final_mse, final_acc_std, final_mse_std = [], [], [], []
        for hdz in range(3): # Bucle para MNIST-FMNIST (ODI)
            with open(f'results/AA_OnlineContDom_Simf2_numN{num_hd[0][hdz]}_numData{num_d[0]}_upType0_det0.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
            final_mse.append(torch.mean(dta[2][:, -1])); final_mse_std.append(torch.std(dta[2][:, -1]))
            final_acc.append(torch.mean(dta[3][:, -1])); final_acc_std.append(torch.std(dta[3][:, -1]))
        ...
        # --- Bloque 2: MHN (Variantes BP) ---
        for d in range(2):
            for o in range(4):
                final_acc, final_mse, final_acc_std, final_mse_std = [], [], [], []
                for hdz in range(3):
                    with open(f'results/AA_OnlineContBP_data{dt[d]}_opt{o}_hdsz{num_hd[d][hdz]}.data','rb') as filehandle:
                        dta = pickle.load(filehandle)
                    final_mse.append(torch.mean(dta[2][:, -1])); final_mse_std.append(torch.std(dta[2][:, -1]))
                    final_acc.append(torch.mean(dta[3][:, -1])); final_acc_std.append(torch.std(dta[3][:, -1]))
        ...
        for o in range(4): # Bucle para MNIST-FMNIST (ODI)
            final_acc, final_mse, final_acc_std, final_mse_std = [], [], [], []
            for hdz in range(3):
                with open(f'results/AA_OnlineContDomBP_opt{o}_hdsz{num_hd[0][hdz]}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                final_mse.append(torch.mean(dta[2][:, -1])); final_mse_std.append(torch.std(dta[2][:, -1]))
                final_acc.append(torch.mean(dta[3][:, -1])); final_acc_std.append(torch.std(dta[3][:, -1]))
        ...
        # --- Bloque 3: Autoencoder (Benchmark) ---
        for d in range(2): # EMNIST y CIFAR
            ae_acc, ae_mse, ae_acc_std, ae_mse_std = [], [], [], []
            for hdz in range(3):
                with open(f'results/AE_Online_data{dt[d]}_hdsz{num_hd[d][hdz]}.data','rb') as filehandle:
                    dta = pickle.load(filehandle)
        ...
        ae_acc, ae_mse, ae_acc_std, ae_mse_std = [], [], [], []
        for hdz in range(3): # MNIST-FMNIST (ODI)
            with open(f'results/AE_Online_data6_hdsz{num_hd[0][hdz]}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
            ae_mse.append(torch.mean(dta[2][:, -1])); ae_mse_std.append(torch.std(dta[2][:, -1]))
            ae_acc.append(torch.mean(dta[3][:, -1])); ae_acc_std.append(torch.std(dta[3][:, -1]))
        ...
        output_dir = 'plots'
        os.makedirs(output_dir, exist_ok=True)
        filename = os.path.join(output_dir, 'OnCont_L1_Final_Performance.png') # Nombre de archivo más descriptivo
        pylab.savefig(filename, bbox_inches='tight') # bbox_inches='tight' para que la leyenda no se corte
        print(f"✅ Gráfico guardado localmente en: {filename}")
        pylab.show()
...
def plot_energies():
    with torch.no_grad():
        ...
        #Small Dataset
        with open(f'results/AA_Online_TreeEnergy_Arch2_max_iter200.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
        ...
        #Larger Dataset
        for t in range(2):
            with open(f'results/AA_Online_TreeEnergy_Arch2_max_iter1000.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
...
def plot_long_cont_acc():
    with torch.no_grad():
        ...
        for d in range(3):
            for hd in range(3):
                with open(f'results/AA_Online_Simf1_numN{hid_sz[hd]}_data{data[d]}_numData8000_upType0_det0.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
...
def plot_sensit():
    with torch.no_grad():
        ...
        # Bloque 1: EMNIST y CIFAR-100 (OCI) para SQHN
        for d in range(2):
            sens_mse = []
            for hdz in range(3):
                # Trainer usa simf=2 para estos tests
                with open(f'results/AA_OnlineCont_Simf2_numN{num_hd[d][hdz]}_data{dt[d]}_numData{num_d[d]}_upType0_det0.data', 'rb') as filehandle:
                    dta_cont = pickle.load(filehandle)
                with open(f'results/AA_Online_Simf2_numN{num_hd[d][hdz]}_data{dt[d]}_numData{num_d[d]}_upType0_det0.data', 'rb') as filehandle:
        ...
        # Bloque 2: MNIST-FMNIST (ODI) para SQHN
        sens_mse = []
        for hdz in range(3):
            # --- CORRECCIÓN FINAL AQUÍ: Trainer usa simf=2 para este test también ---
            with open(f'results/AA_OnlineContDom_Simf2_numN{num_hd[0][hdz]}_numData{num_d[0]}_upType0_det0.data', 'rb') as filehandle:
                dta_cont = pickle.load(filehandle)
            # Este test compara cont=True con cont=False. Trainer usa simf=1 para cont=False, pero simf=2 para cont=True.
            # Necesitamos ser consistentes. El error es que no tenemos los datos para el online de simf=2.
            # Vamos a usar el de simf=1 que sí existe.
            with open(f'results/AA_OnlineContDom_Simf1_numN{num_hd[0][hdz]}_numData{num_d[0]}_upType0_det0online.data', 'rb') as filehandle:
                 dta_on = pickle.load(filehandle)
            cuml_cont = torch.mean(dta_cont[2]); cuml_on = torch.mean(dta_on[2])
            sens_mse.append(torch.abs(torch.mean(cuml_cont - cuml_on)))
        axs[2].plot(num_hd[0], sens_mse, marker='s', alpha=.6, ls='-', label='SQHN', markersize=4.5, color='red')

        # Bloque 3: EMNIST y CIFAR-100 para los modelos BP
        for d in range(2):
            for o in range(4):
                sens_mse = []
                for hdz in range(3):
                    with open(f'results/AA_OnlineContBP_data{dt[d]}_opt{o}_hdsz{num_hd[d][hdz]}.data', 'rb') as filehandle:
                        dta_cont = pickle.load(filehandle)
                    with open(f'results/AA_OnlineBP_data{dt[d]}_opt{o}_hdsz{num_hd[d][hdz]}.data', 'rb') as filehandle:
                        dta_on = pickle.load(filehandle)
                    cuml_cont = torch.mean(dta_cont[2]); cuml_on = torch.mean(dta_on[2])
                    sens_mse.append(torch.abs(torch.mean(cuml_cont - cuml_on)))
                axs[d].plot(num_hd[d], sens_mse, marker='s', alpha=.6, ls=lst[o], label=names[o], markersize=4.5, color=clrs[o])

        # Bloque 4: MNIST-FMNIST para los modelos BP
        for o in range(4):
            sens_mse = []
            for hdz in range(3):
                with open(f'results/AA_OnlineContDomBP_opt{o}_hdsz{num_hd[0][hdz]}.data', 'rb') as filehandle:
                    dta_cont = pickle.load(filehandle)
                with open(f'results/AA_OnlineContDomBP_opt{o}_hdsz{num_hd[0][hdz]}online.data', 'rb') as filehandle:
                    dta_on = pickle.load(filehandle)
                cuml_cont = torch.mean(dta_cont[2]); cuml_on = torch.mean(dta_on[2])
                sens_mse.append(torch.abs(torch.mean(cuml_cont - cuml_on)))
        ...
        output_dir = 'plots'
        os.makedirs(output_dir, exist_ok=True)
        filename = os.path.join(output_dir, 'OnCont_L1_Sensitivity.png')
        pylab.savefig(filename)
        print(f"✅ Gráfico guardado localmente en: {filename}")
        pylab.show()
...
def plot_sensit_tree():
    with torch.no_grad():
        ...
        sens_mse = []
        for hdz in range(3):
            with open(f'results/AATree_OnlineCont_arch2_numN{num_hd[hdz]}_data4_numData2000_wtupType0.data',
                    'rb') as filehandle:
                dta_cont = pickle.load(filehandle)

            with open(f'results/AATree_Online_arch2_numN{num_hd[hdz]}_data4_numData2000_wtupType0.data',
                    'rb') as filehandle:
                dta_on = pickle.load(filehandle)
        ...
        sens_mse = []
        for hdz in range(3):
            with open(f'results/AATree_OnlineContDom_arch2_numN{num_hd[hdz]}_numData2000_wtupType0.data',
                      'rb') as filehandle:
                dta_cont = pickle.load(filehandle)

            with open(f'results/AATree_OnlineContDom_arch2_numN{num_hd[hdz]}_numData2000_wtupType0online.data',
                      'rb') as filehandle:
                dta_on = pickle.load(filehandle)
        ...
        for o in range(4):
            sens_mse = []
            for hdz in range(3):
                with open(f'results/AATreeBP_OnlineCont_arch1_numN{num_hd[hdz]}_data4_numData2000_optim{o}.data', 'rb') as filehandle:
                    dta_cont = pickle.load(filehandle)
                with open(f'results/AATreeBP_Online_arch1_numN{num_hd[hdz]}_data4_numData2000_optim{o}.data', 'rb') as filehandle:
                    dta_on = pickle.load(filehandle)
        ...
        for o in range(4):
            sens_mse = []
            for hdz in range(3):
                with open(f'results/AATreeBP_OnlineContDom_arch1_numN{num_hd[hdz]}_numData2000_optim{o}.data', 'rb') as filehandle:
                    dta_cont = pickle.load(filehandle)
                with open(f'results/AATreeBP_OnlineContDom_arch1_numN{num_hd[hdz]}_numData2000_optim{o}online.data', 'rb') as filehandle:
                    dta_on = pickle.load(filehandle)
                cuml_cont = torch.mean(dta_cont[2])
                cuml_on = torch.mean(dta_on[2])
                sens_mse.append(torch.abs(torch.mean(cuml_cont - cuml_on)))
...
def plot_cont_tree():
    with torch.no_grad():
        ...
        with open(f'results/AATree_OnlineCont_arch2_numN{hid_sz}_data4_numData2000_wtupType0.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
        ...
        for o in range(4):
            with open(f'results/AATreeBP_OnlineCont_arch1_numN{hid_sz}_data4_numData2000_optim{o}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                x = torch.linspace(1, 2000, dta[0].size(1))
        ...
        with open(f'results/AATree_OnlineContDom_arch2_numN{hid_sz}_numData2000_wtupType0.data','rb') as filehandle:
            dta = pickle.load(filehandle)
            x = torch.linspace(1, 2000, dta[0].size(1))
        ...
        for o in range(4):
            with open(f'results/AATreeBP_OnlineContDom_arch1_numN{hid_sz}_numData2000_optim{o}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                x = torch.linspace(1, 2000, dta[0].size(1))
...
def plot_cont_cumul_tree():
    with torch.no_grad():
        ...
        for d in range(2):
            ...
            cuml_mse_std = []
            for hdz in range(3):
                with open(f'results/AATree_OnlineCont_arch2_numN{hid_sz[hdz]}_data4_numData2000_wtupType0.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
        ...
        cuml_mse_std = []
        for hdz in range(3):
            with open(f'results/AATree_OnlineContDom_arch2_numN{hid_sz[hdz]}_numData2000_wtupType0.data','rb') as filehandle:
                dta = pickle.load(filehandle)
            cuml_mse.append(torch.mean(dta[2].sum(1) / dta[2].size(1), dim=0))
            cuml_mse_std.append(torch.std(dta[2].sum(1) / dta[2].size(1), dim=0))
            cuml_acc.append(torch.mean(dta[3].sum(1) / dta[3].size(1), dim=0))
        ...
        for o in range(4):
            ...
            cuml_mse_std = []
            for hdz in range(3):
                with open(f'results/AATreeBP_OnlineCont_arch1_numN{hid_sz[hdz]}_data4_numData2000_optim{o}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                    cuml_mse.append(torch.mean(dta[2].sum(1) / dta[2].size(1), dim=0))
                    cuml_mse_std.append(torch.std(dta[2].sum(1) / dta[2].size(1), dim=0))
                    cuml_acc.append(torch.mean(dta[3].sum(1) / dta[3].size(1), dim=0))
        ...
        for o in range(4):
            ...
            cuml_mse_std = []
            for hdz in range(3):
                with open(f'results/AATreeBP_OnlineContDom_arch1_numN{hid_sz[hdz]}_numData2000_optim{o}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                    cuml_mse.append(torch.mean(dta[2].sum(1) / dta[2].size(1), dim=0))
                    cuml_mse_std.append(torch.std(dta[2].sum(1) / dta[2].size(1), dim=0))
                    cuml_acc.append(torch.mean(dta[3].sum(1) / dta[3].size(1), dim=0))
    ...
    with torch.no_grad():
        ...
        for nst in range(2):
            for plus in range(2):
                ...
                std_rec = []
                for nup in range(3):
                    with open(f'results/AATree_NoisyOnline_numN{hid_sz}_data{data}_numData{max_iter}_NUp{num_up[nup]}_NsType{nst}_plus{bool(plus)}.data', 'rb') as filehandle:
                        dta = pickle.load(filehandle)
                        y_mse.append(torch.mean(dta[0][:, -1]))
                        std_mse.append(torch.std(dta[0][:, -1]))
                        y_rec.append(torch.mean(dta[1][:, -1]))
        ...
        for o in range(2):
            for nst in range(2):
                ...
                std_rec = []
                for nup in range(3):
                    with open(f'results/AATree_NoisyOnlineBP_data{data}_opt{o}_NUp{num_up[nup]}_NsType{nst}.data','rb') as filehandle:
                        dta = pickle.load(filehandle)
                        y_mse.append(torch.mean(dta[0][:, -1]))
                        std_mse.append(torch.std(dta[0][:, -1]))
                        y_rec.append(torch.mean(dta[1][:, -1]))
...
def plot_accTest_tree():
    with torch.no_grad():
        ...
        for sz in range(3):
            with open(f'results/AA_Online_simf2_numN{hd_sz[sz]}_data4_numData{nm_dt[sz]}_upType0_det0.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                l = dta[1].size(1) - 1
        ...
        for arch in range(2):
            for sz in range(3):
                with open(f'results/AATree_Online_arch{archs[arch]}_numN{hd_sz[sz]}_data4_numData{nm_dt[sz]}_wtupType0.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                    l = dta[1].size(1)-1
                    x = torch.linspace(1, nm_dt[sz], l)
                    axs[0,sz].errorbar(x, torch.mean(dta[1][:,0:l], dim=0), yerr=torch.std(dta[1][:,0:l], dim=0), fmt='o', alpha=.6,
...
def plot_emerge():
    with torch.no_grad():
        ...
        sens_mse = []
        with open(f'results/Emerge_arch8_numN1000_data5_numData2000_wtupType0.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
...
def plot_recognition_tree():
    with torch.no_grad():
        ...
        with open(f'results/Recogn_Simf2_numN500_data1_numData3000_upType0_recT0.data', 'rb') as filehandle:
            dta = pickle.load(filehandle)
        ...
        for a in range(2):
            with open(f'results/RecognTree_arch{archs[a]}_data1_numData3000.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                y = (dta[-3] + dta[-2] + dta[-1]) / 3
                axs.errorbar(x, torch.mean(y, dim=0), yerr=torch.std(y, dim=0), fmt='o', alpha=.6,
...
def plot_recognition_tree():
    with torch.no_grad():
        ...
        for k in range(2):
            with open(f'results/Recogn_Simf2_numN500_data1_numData3000_upType0_recT0{ns[k]}.data', 'rb') as filehandle:
                dta = pickle.load(filehandle)
                y = (dta[-3] + dta[-2] + dta[-1]) / 3
                x = torch.linspace(1, 3000, y.size(1))
            ...
            for a in range(2):
                with open(f'results/RecognTree_arch{archs[a]}_data1_numData3000{ns[k]}.data', 'rb') as filehandle:
                    dta = pickle.load(filehandle)
                    y = (dta[-3] + dta[-2] + dta[-1]) / 3
                    axs[k].errorbar(x, torch.mean(y, dim=0), yerr=torch.std(y, dim=0), fmt='o', alpha=.6,
                                       ls='-', label=names[a+1], markersize=2.5, color=clrs[a+1])
